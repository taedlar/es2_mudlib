/*---
description: 這是雪亭鎮客棧的掌櫃，負責開店、關店和保護客棧安全。
...
void create() {
    ...
    if (gt[LT_HOUR] < 7 || gt[LT_HOUR] > 22) {
        on_business = 0;
        move ("/d/snow/inn_staff_room");
    }
    else {
        on_business = 1;
    }
...
void close_inn() {
    ...
    foreach (ob in all_inventory(environment()) ) {
        if (ob == this_object() || !ob->is_character() || ob->id("snow_inn_waiter"))
            continue;
