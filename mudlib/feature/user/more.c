...
void more (string cmd, string *text, int line)
{
    ...
    case ' ':
    case 'f':
        if (sizeof(text) - line >= 22)
            write (HOME CLR);
        write ("\r" + implode (text[line..line+22], "\n"));
        line += 22;
        if (line >= sizeof(text)-1) {
            write ("\n");
            return;
        }
        printf ("\n(More " HIY "%d%%" NOR " / " HIY "%3d" NOR "): ", line*100/sizeof(text), line);
        break;
    case 'b':
        line = (line >= 44) ? (line - 44) : 0;
        write (HOME CLR + implode (text[line..line+22], "\n"));
        line += 22;
        printf ("\n(More " HIY "%d%%" NOR " / " HIY "%3d" NOR "): ", line*100/sizeof(text), line);
        break;
#if 0
    case '/':
        ...
        // if we are in page 1 and it's a new search, start from first line
        if(line == 22 && !match_line)
            line = -1; // incremented below
    ...
    case '?':
    case 'h':
        write ("\n[SPACE,f,ENTER:下頁  b:上頁  q:離開]: ");
        break;
    default:
        write ("\n錯誤指令﹕'" + cmd[0] + "' [SPACE,f,ENTER:下頁  b:上頁  q:離開]: ");
    }
